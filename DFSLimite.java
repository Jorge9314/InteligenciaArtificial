...
public class DFSLimite implements Problem<Point, Action>{
    Laberinto maze;
    
    public DFSLimite(int N_,int M_,int P_,Point inicio_, Point salida_){
        maze = new Laberinto(N_,M_,P_,inicio_,salida_);
    }
    ...
    public static class Laberinto{
        ...
        static Hashtable<Point, Integer> profundidad;
        
        public Laberinto(int N_,int M_,int P_,Point inicio_, Point salida_){
            N = N_;
            M = M_;
            P = P_;
            inicio = inicio_;
            ...
            profundidad = new Hashtable<Point, Integer>();
    ...
    public boolean solve() {    
        ...
        ST.push(maze.inicio);
        maze.profundidad.put(maze.inicio,0);
        ...
        while(!ST.isEmpty()){
            ...
            if(isGoal(actual))
                return true;
            if(maze.profundidad.get(actual)>= maze.P){
                continue;
            }
            ...
            for (Action a: acciones ) {
                ...
                maze.profundidad.put(vecino, maze.profundidad.get(actual)+1);
            }
        }    
        return false;                    
    }
